"""Tests for qarin_cmd/root.py -- CLI entry point, model resolution, banner, etc."""
...
class TestRootVersion:
    def test_version_constant(self):
        from qarin_cmd.root import VERSION

        assert isinstance(VERSION, str)
        assert "." in VERSION

    @patch("qarin_cmd.root.get_config")
    def test_cmd_version(self, mock_cfg, capsys):
        from qarin_cmd.root import cmd_version
    ...
    def test_print_banner(self, mock_cfg):
        from qarin_cmd.root import print_banner
...
class TestFetchLocalModels:
    @patch("qarin_cmd.root.httpx.get")
    ...
    @patch("qarin_cmd.root.httpx.get", side_effect=httpx.ConnectError("fail"))
    ...
    @patch("qarin_cmd.root.httpx.get", side_effect=httpx.TimeoutException("timeout"))
...
class TestResolveModel:
    @patch("qarin_cmd.root._fetch_local_models", return_value=[])
    ...
    @patch("qarin_cmd.root._fetch_local_models", return_value=["llama3.2", "codellama"])
    ...
    @patch("qarin_cmd.root._fetch_local_models", return_value=["llama3.2:latest", "codellama:latest"])
    ...
    @patch("qarin_cmd.root._fetch_local_models", return_value=["glm-5:latest"])
    ...
    @patch("qarin_cmd.root._fetch_local_models", return_value=["codellama:latest"])
    ...
    @patch("qarin_cmd.root._fetch_local_models", return_value=["a", "b", "c", "d", "e", "f"])
...
class TestCmdList:
    ...
    def test_cmd_list_success(self, mock_cfg, mock_get):
        from qarin_cmd.root import cmd_list
...
class TestCmdServe:
    ...
    def test_cmd_serve_success(self, mock_cfg, mock_get):
        from qarin_cmd.root import cmd_serve
    ...
    def test_cmd_serve_http_error(self, mock_cfg, mock_get):
        ...
        mock_get.side_effect = httpx.HTTPStatusError(
            "err", request=httpx.Request("GET", "http://test"), response=response
        )
        with pytest.raises(SystemExit):
            cmd_serve(argparse.Namespace())
