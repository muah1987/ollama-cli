...
def test_pyproject_only_has_ollama_cli_entry_point() -> None:
    """Test that pyproject.toml only defines 'ollama-cli', not a bare 'ollama' entry point."""
    pyproject_path = PROJECT_ROOT / "pyproject.toml"
    with open(pyproject_path, "rb") as f:
        data = tomllib.load(f)

    scripts = data.get("project", {}).get("scripts", {})
    assert "ollama-cli" in scripts, (
        "pyproject.toml [project.scripts] must contain 'ollama-cli'"
    )
    assert "ollama" not in scripts, (
        "pyproject.toml [project.scripts] must NOT contain a bare 'ollama' entry "
        "(only 'ollama-cli' should exist)"
    )


def test_install_sh_wrapper_is_ollama_cli() -> None:
    """Test that install.sh creates an 'ollama-cli' wrapper and not a bare 'ollama' wrapper."""
    install_script = PROJECT_ROOT / "install.sh"
    content = install_script.read_text()

    # The WRAPPER variable must point to ollama-cli
    assert 'WRAPPER="$BIN_DIR/ollama-cli"' in content, (
        "install.sh WRAPPER variable should reference 'ollama-cli'"
    )

    # There should be no line creating a bare 'ollama' wrapper
    for line in content.splitlines():
        stripped = line.strip()
        # Skip comments and lines that reference 'ollama-cli' (which is fine)
        if stripped.startswith("#"):
            continue
        if "ollama-cli" in stripped:
            continue
