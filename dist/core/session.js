...
export class Session {
    ...
    async start() {
        ...
        if (qarinMd) {
            try {
                ...
                console.warn(`Failed to read QARIN.md at "${qarinMd}":`, error);
            }
        }
    ...
    static async load(sessionId, path) {
        const loadPath = path ?? join(SESSIONS_DIR, `${sessionId}.json`);
        const resolvedPath = resolve(loadPath);
        const content = await readFile(resolvedPath, "utf-8");
        const data = JSON.parse(content);
        // Rebuild ContextManager
        const context = ContextManager.fromJSON(data.contextManager);
        // Rebuild TokenCounter with full state restoration
        const tokenCounter = TokenCounter.fromJSON(data.tokenCounter);
    ...
    async appendToQarinMd(qarinMdPath, summary) {
        const entry = `\n\n<!-- session:${summary.sessionId} -->\n` +
            ...
            `(prompt: ${summary.promptTokens.toLocaleString()}, ` +
            `completion: ${summary.completionTokens.toLocaleString()})\n`;
        try {
            ...
            // Log but do not throw, to avoid crashing on non-critical QARIN.md failures
            console.warn(`Failed to append session summary to QARIN.md at "${qarinMdPath}":`, error);
        }
    }
}
