...
import type { SharedState, Wave } from "../types/agent.js";
...
class DiagnosticAgent {
  ...
  async analyze(task: string, context: ContextManager): Promise<Record<string, unknown>> {
...
class AnalysisAgent {
  ...
  async findCause(
    ...
    subCtx.addMessage(
      "user",
      `Given this analysis, identify the root cause:\n\n${JSON.stringify(symptoms, null, 2)}`,
    );
...
class SolutionAgent {
  ...
  async generate(
    ...
    subCtx.addMessage(
      "user",
      `Generate a solution for this root cause:\n\n${JSON.stringify(rootCause, null, 2)}`,
    );
...
export class SubagentOrchestrator extends EventEmitter {
  ...
  constructor(
    ...
    context: ContextManager,
    configs?: Partial<Record<string, SubagentConfig>>,
  ) {
    super();
    ...
    this.diagnosticAgent = new DiagnosticAgent(
      ...
      configs?.diagnostic ?? {
        ...
        name: "diagnostic",
        systemPrompt:
          "You are a diagnostic agent. Analyze the task and identify key issues, dependencies, and constraints.",
      },
    );
    ...
    this.analysisAgent = new AnalysisAgent(
      ...
      configs?.analysis ?? {
        ...
        name: "analysis",
        systemPrompt:
          "You are an analysis agent. Given diagnostic results, identify the root cause and underlying patterns.",
      },
    );
    ...
    this.solutionAgent = new SolutionAgent(
      ...
      configs?.solution ?? {
        ...
        name: "solution",
        systemPrompt:
          "You are a solution agent. Generate concrete, actionable fixes with code examples.",
      },
    );
    ...
    this.verificationAgent = new VerificationAgent(
      ...
      configs?.verification ?? {
        ...
        name: "verification",
        systemPrompt:
          "You are a verification agent. Test solutions for correctness, edge cases, and regressions.",
      },
    );
  }

  /** Execute the full 4-wave orchestration */
  async orchestrate(task: string): Promise<SharedState> {
  ...
  private emitWaveEvent(
    wave: number,
    name: string,
    status: SubagentWaveEvent["status"],
  ): void {
    this.emit("subagent:wave", { wave, name, status } satisfies SubagentWaveEvent);
  }
}
