...
class OllamaCliError(Exception):
    ...
    callers can use a single ``except OllamaCliError`` clause to catch all
...
class ProviderError(OllamaCliError):
...
class OllamaError(OllamaCliError):
...
class SessionError(OllamaCliError):
